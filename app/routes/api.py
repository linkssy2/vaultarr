...
from app.services.acquisition_service import download_acquisition, save_acquisition_link


@api_bp.route("/api/games/<int:game_id>/acquisition/link", methods=["POST"])
def api_acquisition_link(game_id):
    payload = request.get_json(silent=True) or {}
    url = str(payload.get("url") or "").strip()
    provider = str(payload.get("provider") or "User-provided URL").strip()
    title = str(payload.get("title") or "").strip()
    if not url:
        return jsonify({"success": False, "message": "Enter a download URL first."}), 400
    try:
        save_acquisition_link(game_id, url, provider, title)
    except Exception as exc:
        return jsonify({"success": False, "message": str(exc)}), 400
    conn = get_connection()
    updated = conn.execute("SELECT * FROM games WHERE id=?", (game_id,)).fetchone()
    conn.close()
    return jsonify({"success": True, "message": "Acquisition link saved.", "game": enrich_game(row_to_dict(updated))})


@api_bp.route("/api/games/<int:game_id>/acquisition/download", methods=["POST"])
def api_acquisition_download(game_id):
    payload = request.get_json(silent=True) or {}
    url = str(payload.get("url") or "").strip()
    provider = str(payload.get("provider") or "User-provided URL").strip()
    if not url:
        conn = get_connection()
        row = conn.execute("SELECT rom_url FROM games WHERE id=?", (game_id,)).fetchone()
        conn.close()
        url = str(row["rom_url"] or "").strip() if row else ""
    if not url:
        return jsonify({"success": False, "message": "Enter or save a download URL first."}), 400
    try:
        saved = download_acquisition(game_id, url, provider)
    except Exception as exc:
        return jsonify({"success": False, "message": str(exc)}), 400
    conn = get_connection()
    updated = conn.execute("SELECT * FROM games WHERE id=?", (game_id,)).fetchone()
    conn.close()
    return jsonify({"success": True, "message": "File downloaded to the acquisitions folder.", "download": saved, "game": enrich_game(row_to_dict(updated))})
